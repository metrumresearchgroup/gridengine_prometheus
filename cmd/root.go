...
func init() {
	...
	RootCmd.PersistentFlags().Int(sgeExecDPortIdentifier, 1301, "Port for the execution daemon in the grid engine")
	...
	RootCmd.PersistentFlags().Int(sgeQmasterPortIdentifier, 1300, "Port for the master scheduling daemon in the grid engine")
...
func setSGEEnvironmentVariables() error {
	...
	err = os.Setenv("SGE_EXECD_PORT", fmt.Sprint(viper.GetInt("sge.execd_port")))

	if err != nil {
		return err
	}

	err = os.Setenv("SGE_QMASTER_PORT", fmt.Sprint(viper.GetInt("sge.qmaster_port")))
